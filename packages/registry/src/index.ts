/**
 * @safekeep/registry — typed access to the Safekeep provider registry.
 *
 * Usage:
 *   import { getProvider, getProvidersByGroup, providers } from '@safekeep/registry'
 *
...
function loadRegistry(): ProviderMap {
  ...
  try {
    ...
    throw new Error(`@safekeep/registry: could not load providers.json — ${err}`)
  }
}
...
export function getProvider(slug: ProviderSlug): Provider {
  ...
  if (!p) throw new Error(`@safekeep/registry: unknown provider slug "${slug}"`)
  return p
}
